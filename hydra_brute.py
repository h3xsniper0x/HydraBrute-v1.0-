...
class AsyncProtocolHandler:
    ...
    def __init__(self, use_ssl: bool = False):
        ...
        if use_ssl:
            ...
            self._ssl_ctx.verify_mode = ssl.CERT_NONE  # تعطيل التحقق من الشهادة لتجنب فشل الاتصال
        else:
            self._ssl_ctx = None
...
class AsyncHTTPBasicHandler(AsyncProtocolHandler):
    def __init__(self, path: str = "/", use_ssl: bool = False):
        ...
        self._baseline_body_hash: Optional[str] = None
        self._endpoint_protected = False

    async def _baseline_probe(self, host: str, port: int, timeout: float) -> None:
        """
        إرسال طلب بدون بيانات اعتماد للتحقق أن الـ endpoint محمي فعلاً.
        إذا رجع 200 بدون Authorization، فالكلمة المرورية ستكون دائماً "صحيحة" → يجب الإحباط.
        """
        try:
            ...
            await writer.drain()
            status, headers, _, body = await _read_http_response(reader, timeout)
            await _close_writer(writer)

            self._baseline_status = status
            self._baseline_body_hash = hashlib.sha256(body[:4096]).hexdigest()

            # شرط الحماية: 401 + WWW-Authenticate: Basic
            auth_hdr = headers.get("www-authenticate", "").lower()
            self._endpoint_protected = (status == 401 and "basic" in auth_hdr)

            if not self._endpoint_protected:
                # طبع تحذير مرة واحدة فقط
                print(UI.color(
                    f"[!] WARNING: {host}:{port}{self.path} returned {status} "
                    f"without credentials. Endpoint does NOT enforce HTTP Basic auth. "
    ...
    async def authenticate(self, host: str, port: int, user: str, password: str, timeout: float) -> bool:
        # فحص التحضير مرة واحدة فقط
        if not self._baseline_checked:
            await self._baseline_probe(host, port, timeout)
            if not self._endpoint_protected:
                return False  # لا تُنتج نتائج كاذبة أصلاً
        ...
        try:
            ...
            # الفشل المؤكد: 401 أو 403
            if status in (401, 403):
                return False

            # نجاح مبدئي: أي شيء آخر (200, 302, 204...)
            # لكن احذر من سيناريو "الصفحة تعود دائماً 200" — نقارن بالـ baseline
            if status == self._baseline_status:
                # إذا كان الـ baseline 401، فالتغيير إلى 200 مؤكد نجاح
                # لكن إذا كان الـ baseline 200 (endpoint غير محمي)، لن نصل هنا أصلاً بسبب _endpoint_protected
                pass

            # التحقق النهائي: هل الجسم مختلف عن استجابة الفشل؟
            # إرسال طلب تحقق بكلمة مرور عشوائية للمقارنة (expensive، لكن موثوق)
            # للسرعة، نكتفي بأن 200/302 مع status != 401/403 = نجاح
            return status < 400  # 2xx, 3xx = نجاح
...
class AsyncHTTPPostHandler(AsyncProtocolHandler):
    def __init__(self, path="/login", user_field="username", pass_field="password",
                 fail_string=None, success_code=None, csrf_field=None,
                 success_url_substring=None, use_ssl=False):
        super().__init__(use_ssl=use_ssl)
        self.path = path if path.startswith("/") else f"/{path}"
        ...
        self._control_status: Optional[int] = None
        self._control_location: Optional[str] = None
        self._control_body_hash: Optional[str] = None
        self._control_body_text: str = ""
    ...
    async def _baseline_calibrate(self, host: str, port: int, timeout: float) -> None:
        fake_user = "zz_nonexistent_" + hashlib.sha256(os.urandom(8)).hexdigest()[:8]
        fake_pass = "zz_wrongpass_" + hashlib.sha256(os.urandom(8)).hexdigest()[:8]
        ...
        try:
            ...
            fields[self.user_field] = fake_user
            fields[self.pass_field] = fake_pass
            ...
            request = (
                ...
                f"Referer: {scheme}://{host}{self.path}\r\n"
            )
            if cookie_header:
            ...
            try:
                ...
                await writer.drain()
                status, headers, _, resp_body = await _read_http_response(reader, timeout)
            finally:
                await _close_writer(writer)

            self._control_status = status
            self._control_location = headers.get("location", "")
            self._control_body_hash = hashlib.sha256(resp_body[:8192]).hexdigest()
            self._control_body_text = resp_body.decode("utf-8", errors="ignore")

            if self.fail_string is not None:
                if self.fail_string not in self._control_body_text:
                    if not self._calibration_warned:
                        print(UI.color(
                            f"[!] FATAL: --fail-string '{self.fail_string}' NOT found in control response.\n"
                            f"    Use --success-url-substring '/my-account' instead.",
                            UI.RED
                        ))
            ...
            print(UI.color(
                f"[*] Calibration OK: control status={status}, "
                f"location='{self._control_location[:60]}'",
                UI.CYAN
            ))
        except asyncio.TimeoutError:
            if not self._calibration_warned:
                print(UI.color("[!] Calibration failed: Network Timeout. Try increasing --timeout to 10.", UI.YELLOW))
                self._calibration_warned = True
            self._calibration_ok = False
        except Exception as e:
            if not self._calibration_warned:
                err_msg = str(e) if str(e) else type(e).__name__
                print(UI.color(f"[!] Calibration failed: {err_msg}", UI.YELLOW))
                self._calibration_warned = True
            self._calibration_ok = False

    def _evaluate(self, status_code: int, headers: Dict[str, str], body: bytes) -> bool:
        if not self._calibration_ok:
            return False

        body_text = body.decode("utf-8", errors="ignore")
        location = headers.get("location", "")
        body_hash = hashlib.sha256(body[:8192]).hexdigest()

        if self.success_code is not None:
            return status_code == self.success_code

        if self.success_url_substring is not None:
            return (self.success_url_substring in location
                    and location != self._control_location)

        if self.fail_string is not None:
            if body_hash == self._control_body_hash:
                return False
            if self.fail_string in body_text:
                return False
            return True

        if status_code != self._control_status:
            if status_code in (301, 302, 303, 307, 308):
                return location != self._control_location
            return True
        if location != self._control_location:
            return True
        if body_hash != self._control_body_hash:
            return abs(len(body) - len(self._control_body_text)) > 32
        return False

    async def authenticate(self, host, port, user, password, timeout):
        if not self._calibrated:
            async with self._calib_lock:
                if not self._calibrated:
                    await self._baseline_calibrate(host, port, timeout)
        ...
        try:
            ...
            return self._evaluate(status, headers, resp_body)
        except (asyncio.TimeoutError, ConnectionRefusedError, ConnectionResetError,
                BrokenPipeError, ssl.SSLError, OSError):
            return False
        except asyncio.CancelledError:
...
class AsyncAuditOrchestrator:
    ...
    async def _worker(self, index, user, passw):
        ...
        async with self.semaphore:
            ...
            if is_valid and not (self.stop_on_found and self.found_credentials):
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    http_post_p.add_argument("--success-url-substring", type=str, default=None,help="Mark SUCCESS if Location header contains this substring (e.g. '/dashboard'). ""MUCH more reliable than --fail-string.")
